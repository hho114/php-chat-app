...
function get_messages ($timestamp) {
	...
	if ($timestamp === false) {
        ...
        $records = $conn->prepare('SELECT mid, name, content, time FROM user, chatlog WHERE uid = user.id ORDER BY time ASC');
        ...
        $records = $conn->prepare("SELECT mid, name, content, time FROM user, chatlog WHERE uid = user.id AND time > '$timestamp' ORDER BY time ASC");
	...
	else if (($msg_num = mysqli_affected_rows($conn)) === 0) { //if nothing is returned
		mysqli_close($conn); //close the database
	...
	for ($i = 0; $i < $msg_num; ++$i) {
		...
		$messages .= 
			'<div class="message" id="message_'.htmlentities($msg['mid'], ENT_QUOTES, 'UTF-8').'">
				<span class="time">('.$msg['time'].')</span> <a href="#">'.htmlentities($msg['name'], ENT_QUOTES, 'UTF-8').' </a>says:
...
function push_message ($msg) {
	...
	$query = "INSERT INTO chatlog (content, uid) VALUES('$msg', $uid)";
	$res = mysqli_query($conn, $query);
	
	if ($res === false || mysqli_affected_rows($conn) != 1) {
		mysqli_close($conn); //close the database
		return false;
	}
	mysqli_close($conn); //close the database
